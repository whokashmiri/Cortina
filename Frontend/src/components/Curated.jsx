const Curated = () => {
    ...
    return (
      <div className="container mx-auto px-4 mt-14 grid justify-center items-center">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 h-48">
          {cards.map((card, index) => (
            <div
              key={index}
              className={`${card.bg} h-64 w-96 rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 border border-gray-200 rotate-6`}
            >
              <h3 className="text-3xl font-bold bg-gradient-to-r from-slate-400 via-yellow-300 to-emerald-600 bg-clip-text text-transparent mb-2">{card.title}</h3>
